...
func TestLastPrompt(t *testing.T) {
	...
	t.Run("single prompt", func(t *testing.T) {
		require.Equal(t, "single", lastPrompt([]proto.Message{{Role: proto.RoleUser, Content: "single"}}))
	})

	t.Run("multiple prompts", func(t *testing.T) {
		require.Equal(t, "last", lastPrompt([]proto.Message{
			{Role: proto.RoleUser, Content: "first"},
			{Role: proto.RoleAssistant, Content: "hallo"},
			{Role: proto.RoleUser, Content: "middle 1"},
			{Role: proto.RoleUser, Content: "middle 2"},
			{Role: proto.RoleUser, Content: "last"},
		}))
	})
}
