...
func newAnim(cyclingCharsSize uint, label string, r *lipgloss.Renderer, s present.Styles) anim {
	// #nosec G115
	...
	if n >= minRampSize && r.ColorProfile() == termenv.TrueColor {
		...
		ramp := present.MakeGradientRamp(n)
		for i, color := range ramp {
			c.ramp[i] = r.NewStyle().Foreground(color)
		}
		c.ramp = append(c.ramp, present.Reverse(c.ramp)...)
	}
