...
func (m *Mods) startCompletionCmd(content string) tea.Cmd {
	...
	return func() tea.Msg {
		var mod Model
		var api API
		var providerCfg providerConfigs
		...
		providerCfg, err = m.prepareProviderConfigs(mod, api, cfg)
		if err != nil {
			return err
		}

		if err := m.applyProxyConfig(cfg, &providerCfg); err != nil {
			return err
		}
		...
		switch mod.API {
		case "anthropic":
			client = anthropic.New(providerCfg.anthropic)
		case "google":
			client = google.New(providerCfg.google)
		case "cohere":
			client = cohere.New(providerCfg.cohere)
		case "ollama":
			client, err = ollama.New(providerCfg.ollama)
		default:
			client = openai.New(providerCfg.openai)
...
func (m *Mods) prepareProviderConfigs(mod Model, api API, cfg *Config) (providerConfigs, error) {
	cfgs := providerConfigs{}
	...
	case "anthropic":
		...
		if err != nil {
			return providerConfigs{}, modsError{err, "Anthropic authentication failed"}
		}
	...
	case "google":
		...
		if err != nil {
			return providerConfigs{}, modsError{err, "Google authentication failed"}
		}
	...
	case "cohere":
		...
		if err != nil {
			return providerConfigs{}, modsError{err, "Cohere authentication failed"}
		}
	...
	case "azure", "azure-ad": //nolint:goconst
		...
		if err != nil {
			return providerConfigs{}, modsError{err, "Azure authentication failed"}
		}
	...
	default:
		...
		if err != nil {
			return providerConfigs{}, modsError{err, "OpenAI authentication failed"}
		}
		cfgs.openai = openai.Config{
			AuthToken: key,
			BaseURL:   api.BaseURL,
...
func (m *Mods) applyProxyConfig(cfg *Config, providerCfg *providerConfigs) error {
	if cfg.HTTPProxy == "" {
		return nil
	}
	...
	providerCfg.openai.HTTPClient = httpClient
	providerCfg.anthropic.HTTPClient = httpClient
	providerCfg.cohere.HTTPClient = httpClient
	providerCfg.ollama.HTTPClient = httpClient
...
func (m Mods) ensureKey(api API, defaultEnv, docsURL string) (string, error) {
	...
	if key == "" && api.APIKeyCmd != "" {
		...
		out, err := exec.CommandContext(m.ctx, args[0], args[1:]...).CombinedOutput()
		if err != nil {
...
func (m Mods) shouldRenderFormattedOutput() bool {
	return isOutputTTY() && !m.Config.Raw
}

func (m *Mods) renderOutputCmd() tea.Cmd {
...
func (m *Mods) renderFormattedOutput() {
